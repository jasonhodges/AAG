...
namespace AccountAtAGlance.Repository
{
    ...
    {
        bool _LocalDataOnly = Boolean.Parse(ConfigurationManager.AppSettings["LocalDataOnly"]);
        ...
        public MarketQuotes GetMarkets()
        {
            ...
            if (_LocalDataOnly)
            {
                ...
                using (DataContext)
                {
                    markets = GetList<MarketIndex>().ToList();
                }
            }
            else
            {
                markets = _StockEngine.GetMarketQuotes(_MarketIndexSymbols);
            }
